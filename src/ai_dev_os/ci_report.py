...
def render_targeted_summary(run_dict: dict[str, Any]) -> str:
    """Render a targeted (related-module) test run dict as Markdown."""
    sel = run_dict.get("selection") or {}
    lines: list[str] = []
    lines.append(f"# Targeted test run `{run_dict.get('run_id', '')}`")
    lines.append("")
    lines.append(f"- **Verdict:** `{run_dict.get('final_verdict', '')}`")
    lines.append(f"- **Selection reason:** {sel.get('reason', 'unknown')}")
    lines.append(f"- **Ran full suite:** {str(run_dict.get('ran_full_suite', False)).lower()}")
    lines.append(
        "- **Tests:** "
        f"passed={run_dict.get('tests_passed')} "
        f"failed={run_dict.get('tests_failed')} "
        f"skipped={run_dict.get('tests_skipped')}"
    )
    lines.append(f"- **Duration:** {_fmt_duration(run_dict.get('duration_seconds'))}s")
    lines.append(f"- **Next action:** {run_dict.get('next_action', '')}")
    lines.append("")
    selected = sel.get("selected_tests") or []
    lines.append(f"## Selected tests ({len(selected)})")
    lines.append("")
    for test in selected:
        lines.append(f"- {test}")
    unmapped = sel.get("unmapped_sources") or []
    if unmapped:
        lines.append("")
        lines.append("## Changed sources with no related test")
        lines.append("")
        for src in unmapped:
            lines.append(f"- {src}")
    return "\n".join(lines) + "\n"
